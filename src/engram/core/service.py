...
Keeper writer, and exposes the four host operations from DESIGN §4.6 plus a few
conveniences (evidence + audit feeds, cron sweep). Host adapters (the text
tutor, a future voice loop, a Marfini adapter) talk only to this surface; they
never touch adapters or the graph directly.
"""
...
class EngramService:
    ...
    async def evidence(self, node_id: str, limit: int = 20) -> list[Evidence]:
        """A node's scored evidence, newest-first — powers the detail panel."""
        return await self.storage.get_evidence(node_id, limit)
