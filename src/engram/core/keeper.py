...
class Keeper:
    ...
    async def consolidate(self, learner_id: str) -> dict[str, Any]:
        ...
        *per-learner* (not per-session) so cross-document ``relates_to`` edges
        ...
        # 3+5. LINK / RESOLVE — fold each item into the graph, applying evidence,
        #      EWMA mastery, confidence + contradiction handling.
        touched: set[str] = set()
        for item in items:
            await self._link_and_resolve(learner_id, item, touched, stats)

        # 4. MERGE (light pass over concept nodes that ended up near-duplicate).
        stats["merged"] = await self._merge_duplicates(learner_id, touched)
    ...
    async def _link_and_resolve(
        ...
        if matches and matches[0][1] >= self.merge_threshold:
            ...
            node_id = existing.id  # type: ignore[assignment]
    ...
    # ------------------------------------------------------------------ #
    async def _merge_duplicates(self, learner_id: str, touched: set[str]) -> int:
