...
async def test_provider_error_propagates():
    """No swallowing: a provider error (e.g. bad api key) must surface, not
    turn into a silently empty graph."""
    class _Completions:
        async def create(self, **kwargs):
    ...
    class _Client:
        chat = _Chat()
    llm = OpenAICompatibleLLM(client=_Client(), role_to_model={"extractor": "m"})
