...
describe('buildSessionExport', () => {
  it('captures messages (with recalled/saved), keeper, and graph', () => {
    const out = buildSessionExport({
      learner: 'demo-1',
      turns: [
        { role: 'user', content: 'q' },
        { role: 'assistant', content: 'a', recalled: 'mem', saved: [{ type: 'utterance', text: 'q' }] },
      ],
      report: null,
    ...
    expect(out.messages[1]).toMatchObject({ role: 'assistant', content: 'a', recalled: 'mem' })
    expect(out.messages[1].saved).toEqual([{ type: 'utterance', text: 'q' }])
