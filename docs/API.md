...
```json
{"id": "uuid", "type": "concept|preference|goal", "label": "str", "summary": "str?",
...
```
**Evidence dict** (in `/evidence`):
```json
{"id": "uuid", "kind": "explained|asked_about|quiz_correct|quiz_wrong|note|struggle|demonstrated",
 "content": "str?", "importance": 0.0, "created_at": "iso8601"}
```
...
React Flow. `focus` optional (a node id or label to center on); `hops` default 1.

### `GET /evidence?node_id=...&limit=20`
→ `{"evidence": [Evidence...]}` newest-first. Powers the node detail panel
("click a node → its evidence list", DESIGN §4.8).
