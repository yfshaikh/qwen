...
The Keeper synthesizes a durable, decaying mastery signal, so the memory arm
skips re-explaining mastered concepts even after they age out of a raw context
window; the baseline re-explains them.
...
`/recall`, `/consolidate`, `/consolidate-sweep`, `/graph`, `/evidence`, `/audit`,
`/tutor/turn`.
