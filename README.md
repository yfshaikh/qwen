...
# lifespan: await host.start()  /  await host.aclose()

await host.log_turn(uid, user_text=q, tutor_reply=a, refs={"lesson_id": lid})
host.consolidate_soon(uid)
text = (await host.memory.recall(uid, q, budget=600)).text_block
```

**HTTP:** prefer your own `/memory/*` routes that call `host.memory.*` and use
Engram’s Pydantic models (`engram.integrations.fastapi.MemGraphResponse`, …) as
